...
class MainWindow(qtw.QWidget, Ui_Form):
    def __init__(self):
        ...
        self.someWidgets=[]#Changed to self.someWidgets so I can set them in my Cycle_Selection function.
        ...
        self.controller.setWidgets(w=self.someWidgets)
        self.controller.calc()#Have to Force Calc or when you press Metric before calculating the program crashes.

        #show the form
        self.show()

    def change_Units(self):
        """This Function is to change the default numbers when changing from Otto Cycle to Diesel Cycle"""
        if self.cycle_Selector.currentIndex() == 1:
            self.le_THigh.setText("1500.0")
            self.le_TLow.setText("50.0")  # Changed this to 50 instead of 25 to fix US Diesel Graph if it is too low the graph looks spiky on the bottom curve.
            self.le_P0.setText("100.0")
            self.le_V0.setText("1.0")
            self.le_CR.setText("6.0")
        else:
            self.le_THigh.setText("3600.0")
            self.le_TLow.setText("500.0")
            self.le_P0.setText("1.0")
    ...
    def cycle_Selection(self):
        """This Function is to change the controller being used when the ComboBox changes indexes."""
        if self.cycle_Selector.currentIndex() == 1: #I have Diesel Cycle Indexed as 1 and Otto Cycle indexed as 0
            self.controller = dieselCycleController()
            self.controller.setWidgets(w=self.someWidgets)
            self.gb_Input.setTitle("Input for Diesel Cycle") #Changing Title of Input Box
            self.controller.calc()#Have to Force Calc or when you press Metric before calculating the program crashes.
        else:
            self.controller=ottoCycleController()
            self.controller.setWidgets(w=self.someWidgets)
            self.gb_Input.setTitle("Input for Air Standard Otto Cycle")#Changing Title of Input Box
            self.controller.calc() #Have to Force Calc or when you press Metric before calculating the program crashes.
